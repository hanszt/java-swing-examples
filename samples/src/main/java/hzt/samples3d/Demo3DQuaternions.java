...
record Matrix4(double... values) {
    ...
    /// Homogeneous Coordinates: In 3D graphics, points are often represented using homogeneous coordinates (x, y, z, w) instead of just (x, y, z). For a 3D point, 'w' is typically 1. This allows all
    ///       transformations (translation, rotation, scaling, and even perspective projection) to be represented as matrix multiplications. When the Vertex v is passed in, it's implicitly treated as (v.x(),
    ///       v.y(), v.z(), 1).
    ///
    ///    2. Matrix-Vector Multiplication: The values array holds the 16 elements of a 4x4 transformation matrix in row-major order. The method performs the multiplication of this 4x4 matrix with the 4D
    ///       homogeneous coordinate vector of v.
    ///
    ///        * final double x = v.x() * values[0] + v.y() * values[1] + v.z() * values[2] + values[3];
    ///           This calculates the new x coordinate. It's the dot product of the input vertex (v.x, v.y, v.z, 1) with the first row of the matrix (values[0], values[1], values[2], values[3]).
    ///        * final double y = v.x() * values[4] + v.y() * values[5] + v.z() * values[6] + values[7];
    ///           This calculates the new y coordinate using the second row of the matrix.
    ///        * final double z = v.x() * values[8] + v.y() * values[9] + v.z() * values[10] + values[11];
    ///           This calculates the new z coordinate using the third row of the matrix.
    ...
    ///   will change based on the original z coordinate.
    ///
    ///    3. Perspective Division:
    ///       return new Vertex(x / w, y / w, z / w);
    ///       Finally, x, y, and z are divided by w. This step is known as perspective division.
    ///        * If w is 1 (for affine transformations), the coordinates remain unchanged.
    ///        * If w is something other than 1 (due to a perspective projection in the Matrix4), this division produces the foreshortening effect, making objects appear smaller as they move further away. This is
    ///          how 3D perspective is achieved on a 2D screen.
    ///
